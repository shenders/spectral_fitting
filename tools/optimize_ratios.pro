Function calc_profiles, data, dens_arr, te_arr, ratio1, ratio2, exp_ratio1, exp_ratio2 ,debug=debug
	...
	idx        = fltarr(num)	
	min2       = fltarr(num)
	te         = fltarr(n_elements(exp_ratio1[*,0]),n_elements(exp_ratio1[0,*]))
	dens       = fltarr(n_elements(exp_ratio1[*,0]),n_elements(exp_ratio1[0,*]))
	sim_ratio1 = fltarr(n_elements(exp_ratio1[*,0]),n_elements(exp_ratio1[0,*]))
	sim_ratio2 = fltarr(n_elements(exp_ratio1[*,0]),n_elements(exp_ratio1[0,*]))
	...
	for k=0,n_elements(exp_ratio1[0,*])-1 do begin
		...
		for l=0,n_elements(order)-1 do begin
			chi[l] = (exp_ratio1[i,k] - ratio1[idx[order[k]],order[k]])^2/exp_ratio1[i,k]
			chi[l] = chi[l] +  (exp_ratio2[i,k] - ratio2[idx[order[k]],order[k]])^2/exp_ratio2[i,k]
		endfor
		...
		if abs(sim_ratio1[i,k]-exp_ratio1[i,k])/exp_ratio1[i,k] lt tol and $
		   abs(sim_ratio2[i,k]-exp_ratio2[i,k])/exp_ratio2[i,k] lt tol then begin
...
Function optimize_ratios,data,shot,los,transmission,sm=sm,debug=debug,nocn=nocn,jet=jet,horizontal=horizontal
	...
	if keyword_set(docalc) then begin
	    num      = 40
    	...
    	endif else restore,file='save/atomdb/ratios.sav'
	...
	cn_upper = exp_ratio1
        ...
        if ~keyword_set(nocn)then begin
    	    cn_1 = calc_cn(shot,los,transmission,te_lower,dens_lower,data,sm,err=err_1,jet=jet,horizontal=horizontal)
            cn_2 = calc_cn(shot,los,transmission,te_lower,dens_upper,data,sm,err=err_2,jet=jet,horizontal=horizontal)
            cn_3 = calc_cn(shot,los,transmission,te_upper,dens_lower,data,sm,err=err_3,jet=jet,horizontal=horizontal)
            cn_4 = calc_cn(shot,los,transmission,te_upper,dens_upper,data,sm,err=err_4,jet=jet,horizontal=horizontal)
	    for k=0,n_elements(exp_ratio1[0,*])-1 do begin
	    	for i=0,n_elements(data.time)-1 do begin
