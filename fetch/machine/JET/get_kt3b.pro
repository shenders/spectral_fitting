Function get_kt3b,pulse,debug=debug,spec=spec,rrange=rrange,psplot=psplot
    if ~keyword_set(spec)then spec='kt3b'
    data = agm_readspec(pulse,spec=spec)
    agm_process_data,data,/wavecal,/radcal,/calibrate,/cal_si
    wave = data.data.wave_cal
    wave = REVERSE(wave,1)
    emis = data.data.data
    emis = REVERSE(emis,1)
    time = data.data.time ; this is the mid-point time (i.e. start point + end point / 2.0)
...
; Load in vessel details
    ...
    if keyword_set(debug)then begin
    	if keyword_set(psplot)then makeps,file='kt3b_geom.ps',xs=8,ys=5
