Function get_kt3a,pulse,debug=debug,spec=spec,rrange=rrange,psplot=psplot
    if ~keyword_set(spec)then spec='kt3a'
...
; Load in vessel details
    ...
    if keyword_set(debug)then begin
    	if keyword_set(psplot)then makeps,file='kt3a_geom.ps',xs=8,ys=5
