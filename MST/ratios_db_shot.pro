Function get_ratios,shot,los=los,interelm=interelm,minemiss=minemiss,trange=trange
	if ~keyword_set(los)then los='ROV014'
	if ~keyword_set(trange)then trange=[0,10]
	if ~keyword_set(minemiss)then minemiss=0
	...
	endif else id=findgen(n_elements(output.time))
	...
	id     = where(nii3995 gt minemiss and time ge trange[0] and time le trange[1])

return,{ratio1:ratio1[id],ratio2:ratio2[id]}
End

Pro ratios_db_shot,shot,los=los,showdens=showdens,psplot=psplot,interelm=interelm,trange=trange
!quiet=1
num    = 100
...
collos  = [colors.blue,colors.green,colors.red,colors.orange,colors.magenta]
for j=0,n_elements(los)-1 do begin
	for i=0,n_elements(shot_db)-1 do begin
		x=get_ratios(shot_db[i],los=los[j],interelm=interelm,trange=trange)
