...
End
PRO iaea_30776,psplot=psplot,shot=shot,rawelms=rawelms,xr=xr,yr=yr,elmcond=elmcond
	...
	if keyword_set(rawelms)then telm    = find_elms(shot,t10) else telm= find_elm(shot,t10)
	...
	xtitle='Time [s]',ytitle='[eV]' & legend,'(e) Tdiv',xr,[0,30],colors.black
	...
	sv = 15
	plot,t08,smooth(y10,sv,/edge_trun),col=colors.black,xs=8,backg=colors.white,yr=yr,position=graphpos(0,3,nrow,ncol,xspc=xspc,yspc=yspc),xr=xr,ytitle='[10!u18!n ph/s/m!u2!n/sr]',xtickname=replicate(' ',30)
	...
	yr = [0,30]
	user_psym,5 & plot,t10,n10,psym=8,col=colors.black,xs=8,backg=colors.white,yr=yr,position=graphpos(0,2,nrow,ncol,xspc=xspc,yspc=yspc),xr=xr,ytitle='[10!u18!n ph/s/m!u2!n/sr]',xtickname=replicate(' ',30)
	user_psym,5,/fill & oplot,t10[id],n10[id],psym=8,col=colors.black & legend,'(c) ROV010',xr,yr,colors.black

	yr = [0,30]
	user_psym,5 & plot,t12,n12,psym=8,col=colors.black,xs=8,backg=colors.white,yr=yr,position=graphpos(0,1,nrow,ncol,xspc=xspc,yspc=yspc),xr=xr,ytitle='[10!u18!n ph/s/m!u2!n/sr]',xtickname=replicate(' ',30)
	user_psym,5,/fill & oplot,t12[id],n12[id],psym=8,col=colors.blue & legend,'(b) ROV012',xr,yr,colors.black

	yr = [0,15]
	user_psym,5 & plot,t14,n14,psym=8,col=colors.black,xs=0,backg=colors.white,yr=yr,position=graphpos(0,0,nrow,ncol,xspc=xspc,yspc=yspc),title='AUG #30776',xr=xr,ytitle='[10!u18!n ph/s/m!u2!n/sr]',xtickname=replicate(' ',30)
