Pro wmhd,debug=debug,tdiv=tdiv,traces=traces
...
;	tdiv = 8
	...
	if keyword_set(traces)then begin
		...
		for i=0,n_elements(shots)-1 do begin
			...
			if swch eq 1 then begin
				...
				read_signal_mrm,0L,shots[i],'FPG','Wmhd',tline,wmhd,2
				ymax = 1.4
				plot, tline, wmhd/1e6, xr=xrange, xs=1, yr=[0,ymax],ys = 1, /nodata, col=colors.black, back=colors.white,$
				title=string(shots[i],format='("Wmhd [black], H98 [Blue]: ",I5)'),xtitle='Time [s]', ytitle='[MW]'
				...
				oplot,tline, wmhd/1e6, col=colors.black
				
				hfac = h98(shots[i])
				
				oplot,hfac.time,hfac.h98,col=colors.blue
			endif
		endfor
		ref = 0
		for i=0,n_elements(shots)-1 do begin
	...
	for i=0,n_elements(shots)-1 do begin
		...
		if shots[i] eq 35381 and swch eq 1 then begin
			read_signal_mrm,0L,35158,'FPG','Wmhd',tline,wmhd,2
		endif else begin
			read_signal_mrm,0L,shots[i],'FPG','Wmhd',tline,wmhd,2
		end
		bindata,tline,wmhd,[times[i],times[i]+ twin[i]],avr_wm,err_wm
		if swch eq 1 then begin
			ref_wm_avr = avr_wm
			ref_wm_err = err_wm
		endif
